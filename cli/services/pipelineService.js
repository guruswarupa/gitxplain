  ...
  if (context.type === "node") {
    ...
    if (packaging.deb || packaging.homebrew || packaging.aur) {
      return [
        ...
        `            echo "${packageName}@\${VERSION} is already published on npm. Skipping npm publish."`,
        "          else",
        ...
        `            if curl -fsSL "\${TARBALL_URL}" -o "${packageName}-\${VERSION}.tgz"; then`,
        "              break",
        "            fi",
        "",
    ...
    return [
      ...
      "        run: |",
      "          VERSION=\"${{ steps.meta.outputs.version }}\"",
      `          if npm view "${context.packageJson?.name ?? context.displayName ?? "package"}@\${VERSION}" version >/dev/null 2>&1; then`,
      "            echo \"published=true\" >> \"${GITHUB_OUTPUT}\"",
      `            echo "${context.packageJson?.name ?? context.displayName ?? "package"}@\${VERSION} is already published on npm. Skipping npm publish."`,
      "          else",
      "            echo \"published=false\" >> \"${GITHUB_OUTPUT}\"",
      "          fi",
      "      - name: Publish to npm",
